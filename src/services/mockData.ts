...
export const generateMockTransactions = (): TransactionResource[] => {
  ...
  // Netflix - Monthly - 15th - $16.99
  for (let i = 0; i < 6; i++) {
    const date = subDays(today, i * 30 + 5); // Rough monthly
    transactions.push(createTransaction("Netflix", 16.99, date, "streaming"));
  }

  // Spotify - Monthly - 2nd - $12.99
  for (let i = 0; i < 6; i++) {
    const date = subDays(today, i * 30 + 15);
    transactions.push(createTransaction("Spotify", 12.99, date, "music"));
  }

  // Adobe - Monthly - 20th - $54.99 (Price change scenario: was 49.99 3 months ago)
  for (let i = 0; i < 3; i++) {
  ...
  // AWS - Monthly - Variable amount
  for (let i = 0; i < 6; i++) {
    const date = subDays(today, i * 30 + 10);
    const amount = 30 + Math.random() * 5; // Variable
