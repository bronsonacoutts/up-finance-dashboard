...
describe("detectSubscriptions", () => {
  ...
  it("should detect price increases", () => {
    const today = new Date();
    const transactions = [
      createTx("Netflix", 18.99, subDays(today, 5).toISOString()),
      createTx("Netflix", 16.99, subDays(today, 35).toISOString()), // Price change here
      createTx("Netflix", 16.99, subDays(today, 65).toISOString()),
    ];

    const subs = detectSubscriptions(transactions);
    expect(subs).toHaveLength(1);
    expect(subs[0].priceChange).toBeDefined();
    expect(subs[0].priceChange?.amount).toBeCloseTo(2.00);
    expect(subs[0].priceChange?.percentage).toBeCloseTo(11.77, 1);
  });
});
